	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussfangkongzhishi`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(this.ruleForm.id){
					await this.$api.update(`discussfangkongzhishi`, this.ruleForm);
				}else{
					await this.$api.add(`discussfangkongzhishi`, this.ruleForm);
				}
				uni.setStorageSync('discussfangkongzhishiCleanType',true);

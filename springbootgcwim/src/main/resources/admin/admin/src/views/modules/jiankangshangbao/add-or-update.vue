<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="当日体温" prop="dangritiwen">
          <el-input v-model="ruleForm.dangritiwen" 
              placeholder="当日体温" clearable  :readonly="ro.dangritiwen"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="当日体温" prop="dangritiwen">
              <el-input v-model="ruleForm.dangritiwen" 
                placeholder="当日体温" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否不适" prop="shifoubushi">
          <el-select v-model="ruleForm.shifoubushi" placeholder="请选择是否不适">
            <el-option
                v-for="(item,index) in shifoubushiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="是否不适" prop="shifoubushi">
	      <el-input v-model="ruleForm.shifoubushi"
                placeholder="是否不适" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="有无发热" prop="youwufare">
          <el-select v-model="ruleForm.youwufare" placeholder="请选择有无发热">
            <el-option
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="异常情况" prop="yichangqingkuang">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="异常情况"
                  v-model="ruleForm.yichangqingkuang" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.yichangqingkuang" label="异常情况" prop="yichangqingkuang">
                    <span>{{ruleForm.yichangqingkuang}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  data() {
	...
	userid : false,
      ...
      rules: {
          ...
          dangritiwen: [
                { required: true, message: '当日体温不能为空', trigger: 'blur' },
          ],
          shifoubushi: [
          ],
          youwufare: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhuzhi = true;
          ...
          if(o=='dangritiwen'){
            this.ruleForm.dangritiwen = obj[o];
	    this.ro.dangritiwen = true;
            continue;
          }
          if(o=='shifoubushi'){
            this.ruleForm.shifoubushi = obj[o];
	    this.ro.shifoubushi = true;
            continue;
          }
          if(o=='youwufare'){
	    ...
	    this.ro.youwufare = true;
          ...
          if(o=='yichangqingkuang'){
            this.ruleForm.yichangqingkuang = obj[o];
	    this.ro.yichangqingkuang = true;
            continue;
          }
          if(o=='fabushijian'){
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.shifoubushiOptions = "是,否".split(',')
            this.youwufareOptions = "有,无".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `jiankangshangbao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiankangshangbao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiankangshangbaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
